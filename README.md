...
Расширение работает в 4 шага:

1. **WebUI отдаёт JS** — браузер загружает `polza-balance.js` со страницей
2. **fetch() к Polza API** — виджет напрямую запрашивает баланс и историю
3. **JSON ответ** — Polza присылает баланс, историю генераций
4. **Рендер** — виджет рисует баланс в боковой панели и попап с расходами
